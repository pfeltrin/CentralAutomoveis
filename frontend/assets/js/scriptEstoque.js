...
// ==============================
//   EDITAR VEÍCULO (VERSÃO FINAL)
// ==============================

// Limpa o foco para não travar o teclado no Electron
function resetarFocoElectron() {
...
async function abrirEdicao(veiculo) {
    ...
    // Garante que não existam modais antigos abertos
    const antigo = document.querySelector(".modalEdicao");
    ...
    modal.innerHTML = `
        <div class="conteudoModal" style="-webkit-app-region: no-drag;">
    ...
    // FOCO AUTOMÁTICO (O pulo do gato para o Electron)
    setTimeout(() => {
        const input = document.getElementById("editMarca");
        if (input) {
            input.focus();
            input.select();
        }
    ...
    document.getElementById("btnSalvarEdicao").onclick = async () => {
        ...
        try {
            ...
            if (res.ok) {
                modal.remove();
                resetarFocoElectron();
                // RELOAD É OBRIGATÓRIO PARA DESTRAVAR O TECLADO NO ELECTRON
                window.location.reload(); 
            } else {
...
function adicionarEventosVendido() {
    document.querySelectorAll(".btnVendido").forEach(btn => {
        btn.addEventListener("click", async function () {
            ...
            // O confirm do Electron às vezes trava o foco. 
            // Vamos garantir que o foco saia do botão antes de abrir o confirm.
            this.blur(); 
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    // LIMPEZA DE FOCO ANTES DO RELOAD
